...
def draw_detections(image, boxes, scores, class_ids, mask_alpha=0.1):
    det_img = image.copy()
...
def draw_masks(image: np.ndarray, boxes: np.ndarray, classes: np.ndarray, mask_alpha: float = 0.1) -> np.ndarray:
    mask_img = image.copy()
